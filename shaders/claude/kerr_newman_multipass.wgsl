...
fn gen_disk_noise(pos: vec3<f32>, start_lvl: f32, end_lvl: f32, contrast: f32) -> f32 {
    ...
    let max_iter = min(i_end - i_start, 4);
...
fn mainImage(frag_coord: vec2<f32>) -> vec4<f32> {
    ...
    // Pixel UV (footer already flips Y to Shadertoy convention)
    let pixel_uv = (frag_coord - 0.5 * res) / res.y;
    ...
    // Temporal smoothing via self-feedback (reduces noise, builds quality over frames)
    if (u.iFrame > 4) {
        let prev = stSample(iChannel3, uv).xyz;
        // Stronger blend: 0.35 means we keep 65% new + 35% old per frame
        // This builds up quality over several frames even at low FPS
        let blend = 0.35;
        final_col = mix(final_col, prev, blend);
    }

    // Output raw HDR — tone mapping happens in Image pass
...
// Extract only bright pixels for bloom (threshold removes dark background/stars)
fn bloom_extract(c: vec3<f32>) -> vec3<f32> {
    let lum = 0.2126 * c.r + 0.7152 * c.g + 0.0722 * c.b;
    let threshold = 0.08;
    let knee = max(0.0, lum - threshold) / max(lum, 0.0001);
    return c * clamp(knee, 0.0, 1.0);
}

fn mainImage(frag_coord: vec2<f32>) -> vec4<f32> {
    let uv = frag_coord / u.iResolution.xy;
    let px = 1.0 / u.iResolution.x;

    // Wide horizontal blur on thresholded bright pixels only
    // Exponentially spaced taps: 2, 5, 12, 28, 60, 120, 200 pixels
    var color = bloom_extract(stSample(iChannel0, uv).xyz) * 0.16;

    color += (bloom_extract(stSample(iChannel0, uv + vec2<f32>(2.0 * px, 0.0)).xyz)
            + bloom_extract(stSample(iChannel0, uv - vec2<f32>(2.0 * px, 0.0)).xyz)) * 0.14;
    color += (bloom_extract(stSample(iChannel0, uv + vec2<f32>(5.0 * px, 0.0)).xyz)
            + bloom_extract(stSample(iChannel0, uv - vec2<f32>(5.0 * px, 0.0)).xyz)) * 0.12;
    color += (bloom_extract(stSample(iChannel0, uv + vec2<f32>(12.0 * px, 0.0)).xyz)
            + bloom_extract(stSample(iChannel0, uv - vec2<f32>(12.0 * px, 0.0)).xyz)) * 0.09;
    color += (bloom_extract(stSample(iChannel0, uv + vec2<f32>(28.0 * px, 0.0)).xyz)
            + bloom_extract(stSample(iChannel0, uv - vec2<f32>(28.0 * px, 0.0)).xyz)) * 0.07;
    color += (bloom_extract(stSample(iChannel0, uv + vec2<f32>(60.0 * px, 0.0)).xyz)
            + bloom_extract(stSample(iChannel0, uv - vec2<f32>(60.0 * px, 0.0)).xyz)) * 0.05;
    color += (bloom_extract(stSample(iChannel0, uv + vec2<f32>(120.0 * px, 0.0)).xyz)
            + bloom_extract(stSample(iChannel0, uv - vec2<f32>(120.0 * px, 0.0)).xyz)) * 0.03;
    color += (bloom_extract(stSample(iChannel0, uv + vec2<f32>(200.0 * px, 0.0)).xyz)
            + bloom_extract(stSample(iChannel0, uv - vec2<f32>(200.0 * px, 0.0)).xyz)) * 0.02;

    return vec4<f32>(color, 1.0);
}

// ============================================================================
// PASS: Separable Gaussian Blur (Vertical)
// Smooths the multi-scale bloom with wide Gaussian kernel
// ============================================================================
...
fn mainImage(frag_coord: vec2<f32>) -> vec4<f32> {
    let uv = frag_coord / u.iResolution.xy;
    let py = 1.0 / u.iResolution.y;

    // Wide vertical blur (matching horizontal tap structure)
    var color = stSample(iChannel0, uv).xyz * 0.16;

    color += (stSample(iChannel0, uv + vec2<f32>(0.0, 2.0 * py)).xyz
            + stSample(iChannel0, uv - vec2<f32>(0.0, 2.0 * py)).xyz) * 0.14;
    color += (stSample(iChannel0, uv + vec2<f32>(0.0, 5.0 * py)).xyz
            + stSample(iChannel0, uv - vec2<f32>(0.0, 5.0 * py)).xyz) * 0.12;
    color += (stSample(iChannel0, uv + vec2<f32>(0.0, 12.0 * py)).xyz
            + stSample(iChannel0, uv - vec2<f32>(0.0, 12.0 * py)).xyz) * 0.09;
    color += (stSample(iChannel0, uv + vec2<f32>(0.0, 28.0 * py)).xyz
            + stSample(iChannel0, uv - vec2<f32>(0.0, 28.0 * py)).xyz) * 0.07;
    color += (stSample(iChannel0, uv + vec2<f32>(0.0, 60.0 * py)).xyz
            + stSample(iChannel0, uv - vec2<f32>(0.0, 60.0 * py)).xyz) * 0.05;
    color += (stSample(iChannel0, uv + vec2<f32>(0.0, 120.0 * py)).xyz
            + stSample(iChannel0, uv - vec2<f32>(0.0, 120.0 * py)).xyz) * 0.03;
    color += (stSample(iChannel0, uv + vec2<f32>(0.0, 200.0 * py)).xyz
            + stSample(iChannel0, uv - vec2<f32>(0.0, 200.0 * py)).xyz) * 0.02;

    return vec4<f32>(color, 1.0);
}

// ============================================================================
// PASS: Final Composite + Tone Mapping
// Matches GLSL Image.txt: base + bloom composite with filmic tone mapping
// ============================================================================
...
fn mainImage(frag_coord: vec2<f32>) -> vec4<f32> {
    ...
    let base = stSample(iChannel0, uv).xyz;
    let bloom = stSample(iChannel1, uv).xyz;

    // Composite: add thresholded bloom with slight warm tint
    let warm_bloom = bloom * vec3<f32>(1.15, 1.05, 0.9);
    var color = base + warm_bloom * 1.8;

    // Tone mapping (matching GLSL Image.txt)
    // Pre-expand dynamic range for Reinhard
    color = pow(max(color, vec3<f32>(0.0)), vec3<f32>(1.5));
    // Reinhard compression
    color = color / (vec3<f32>(1.0) + color);
    // Un-expand
    color = pow(max(color, vec3<f32>(0.0)), vec3<f32>(1.0 / 1.5));

    // S-curve contrast
    color = color * color * (vec3<f32>(3.0) - 2.0 * color);

    // Warm color grading (GLSL: pow(color, vec3(1.3, 1.20, 1.0)))
    color = pow(max(color, vec3<f32>(0.0)), vec3<f32>(1.3, 1.20, 1.0));
    color = clamp(color * 1.01, vec3<f32>(0.0), vec3<f32>(1.0));

    // Gamma (GLSL uses 0.7/2.2 for brighter output)
    color = pow(max(color, vec3<f32>(0.0)), vec3<f32>(0.7 / 2.2));
